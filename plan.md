...
- 마크다운 문서를 블로그처럼 렌더링해 GitHub Pages로 배포
...
### Phase 2 — 블로그 스타일 커스터마이징 (1~2일)

**목표**: 블로그처럼 보이도록 테마와 네비게이션 구성

1. VitePress 테마 설정 (`config.ts`)
   - 사이트 타이틀, 설명, 소셜 링크
   - 사이드바 자동 생성 (카테고리별)
   - 상단 네비게이션 (홈, 카테고리 목록)
2. 홈 랜딩 페이지 (`index.md`) 디자인
   - 최근 문서 목록 또는 카테고리 그리드
3. 커스텀 CSS로 블로그 느낌 조정 (폰트, 간격 등)
4. 마크다운 플러그인 추가 (수식, 코드 하이라이트 등 필요 시)

**완료 기준**: 카테고리 구조와 네비게이션이 블로그처럼 동작
